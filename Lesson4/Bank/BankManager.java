...
public class BankManager {
    ...
    private static Queue<String> clientQueue = new LinkedList<String>();
    private static Map<String, Integer> clientMap = new LinkedHashMap<String, Integer>();
    private static Set<String> clientSet = new LinkedHashSet<String>();
    ...
    public void registerNewClient(Client client) throws WrongName {
        ...
        if (clientSet.contains(client.getFullName()) == false) {

            for (String fullName : clientSet) {
                int counterFullName = Levenshtein.calculate(client.getFullName(), fullName);
                if (counterFullName <= 2) {
                    throw new WrongName("Человек с введенными данным уже является клиентом нашего банка!");
                }
            }
    ...
    public static Queue<String> getClientQueue() {
        return clientQueue;
    }
}
